...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer4 = self._make_layer(block, 32, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], norm=nn.Identity)
        #self.layer4 = nn.Conv2d(16, 4, 1, 2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.fc = nn.Conv2d(32, 4, 1)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, norm=None):
        norm_layer = norm if norm is not None else self._norm_layer
        downsample = None
