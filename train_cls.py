...
iterator_val = ds.GenerateIterator_cls(args.val_image_pth, eval=True, duplicate_dataset=1)
...
def train(seg_and_cls, use_seg_ratio, exp_iter):
    ...
    model.fc = models.domain_classifier(args.seg_features * 4 * 4,  args.num_classes)
    model.fc = torch.nn.Sequential(
        models.Flatten(),
        torch.nn.Linear(4096, args.num_classes),
    )
    ...
    args.experiment_save_pth = '{}_cls/{}'.format(args.experiments_pth, args.experiment_time)
    ...
    for epoch in range(start_epoch, 1+args.num_epoch):
        ...
        for batch_it, (image, label, mask, is_cls) in enumerate(progress_bar):
            ...
            if args.seg_and_cls:
                if cls:
                    loss_cls = lossfn_cls(model.fc(feature_maps_l[is_cls]), label[is_cls])
                    sum_loss_cls += loss_cls.item()

            ''' segmentation '''
            if seg:
                loss_seg = lossfn_seg(feature_maps_h[~is_cls], mask[~is_cls])
                sum_loss_seg += loss_seg.item()

            if not isinstance(loss_cls+loss_seg, int):
                optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    for use_seg_ratio in tqdm(reversed([0.10, 0.20, 0.30, 0.40, 0.50, 0.75, 1])):
        for exp_iter in [1]:
            for seg_and_cls in [1]:
                print('================== Seg ratio {:.2f} ==================='.format(use_seg_ratio))
